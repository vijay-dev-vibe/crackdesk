...
import { PLAN_LIMITS, getRemainingTests, getRemainingInterviews, type PlanType } from "@/lib/plans";

export function useSubscription() {
  const [subscription, setSubscription] = useState<{ plan_type: PlanType } | null>(null);
  ...
  const incrementMockTestUsage = useCallback(() => {
    ...
    const incrementAIInterviewUsage = useCallback(async () => {
      ...
      const { error } = await supabase
        .from("interview_results")
        .insert({ user_id: session.user.id, created_at: new Date().toISOString() });
